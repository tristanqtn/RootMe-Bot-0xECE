...
def reduce_points(username, points_to_deduct):
    ...
    conn = sqlite3.connect(
        "rootme_data.db"
    )  # Remplacez par le chemin de votre base de données SQLite
    cursor = conn.cursor()
    ...
    new_points = max(
        current_points - points_to_deduct, 0
    )  # Ne pas descendre en dessous de 0

    # Met à jour les points dans la base de données
    cursor.execute(
        "UPDATE user_data SET points = ? WHERE username = ?", (new_points, username)
    )
    conn.commit()
